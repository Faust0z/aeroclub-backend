...
from app.controllers.flight_invoices import RecibosController
...
def obtenerRecibos(email):
    try:
        ...
        if respuesta == 1:
            return jsonify(
                {
                    "message": "El mail no le pertenece a un asociado",
                    "success": False,
                }
            )
        if respuesta == 2:
...
def crear_recibos_vuelos():
    try:
        ...
        recibosController = RecibosController()
        respuesta = recibosController.crearRecibo(
            emailAsociado,
            emailInstructor,
            emailGestor,
            observaciones,
        ...
        if respuesta == 1:
            return jsonify(
                {
                    "mensaje": "El asociado que ingreso, no tiene rol de Asociado",
                    "success": False,
                }
            )
        if respuesta == 2:
            return jsonify(
                {
                    "mensaje": "El gestor que ingreso, no tiene rol de Gestor",
                    "success": False,
                }
            )
        if respuesta == 3:
            return jsonify(
                {
                    "mensaje": "El instructor que ingreso, no tiene rol de instrunctor",
                    "success": False,
                }
            )
        if respuesta == 4:
        ...
        if respuesta == 5:
            return jsonify(
                {
                    "message": "Algún itinerario tiene un dato erroneo",
                    "success": False,
                }
            )
        if respuesta == 6:
        ...
        if respuesta == 7:
            return jsonify(
                {
                    "message": "Hubo un error en la transacción verifique los dato y envie nuevamente",
                    "success": False,
                }
            )
        if respuesta == 8:
            return jsonify(
                {
                    "message": "Algún dato de los itinerarios es erroneo, veifique los dato y envie nuevamente",
                    "success": False,
                }
            )
        if respuesta == 9:
            return jsonify(
                {
                    "message": "La aeronave no es válida, ingrese una matrícula válida",
                    "success": False,
                }
            )
        if respuesta == 10:
        ...
        if respuesta[0] == 13:
            ...
            if respuesta:
                ...
                return jsonify(
                    {
                        "message": "Se creó el recibo pero no se envio el mail por algun motivo",
                        "success": True,
                    }
                )
