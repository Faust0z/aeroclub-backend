    ...
    if has_access:
        try:
            ...
            respuesta = recibosController.crearRecibo(
                emailGestor, monto, observaciones, tipoPago, motivo
            )
            if respuesta == 1:
                return jsonify(
            ...
            if respuesta == 3:
                return jsonify(
                    {
                        "mensaje": "No se pudo crear el recibo, intente nuevamente verificando los datos",
                        "success": False,
                    }
                )
            if respuesta == 4:
            ...
            if respuesta == 6:
                return jsonify(
                    {
                        "message": "Ocurrio un error, verifiqué los datos y pruebe nuevamente",
                        "success": False,
                    }
                )
            if respuesta == 13:
