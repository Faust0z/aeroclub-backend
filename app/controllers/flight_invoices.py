            ...
            if aeronave:
                ...
                for itinerario in itinerarios:
                    ...
                    salida = datetime.strptime(
                        itinerario.get("horaSalida"), "%Y-%m-%d %H:%M:%S"
                    )
                    llegada = datetime.strptime(
                        itinerario.get("horaLlegada"), "%Y-%m-%d %H:%M:%S"
                    )
                ...
                # pago del asociado
                transaccionAsociado = cuentaCorrienteController.actualizar_saldo(
                    cuentaCorrienteController,
                    asociado.id_usuarios,
                    precioTotalVuelo,
                    fecha,
                    "Pago de vuelo",
                    "Efectivo",
                    ...
                    if recibo:
                        ...
                        for itinerario in itinerarios:
                            ...
                            respuestaCreacionItinerario = (
                                ItinerariosController.crearItinerario(
                            ...
                            itinerariosCreadosConSuRelacion.append(
                                respuestaCreacionItinerario
                            )
                        ...
                        if not respuestaCreacionItinerario[0]:
                            ...
                            if instructor:
                                if transaccionInstructor:
                                    ...
                                    idCuentaCorrienteInstructor = (
                                        transaccionInstructor.cuenta_corriente_id
                                    )
                                    db.session.delete(transaccionInstructor)
                                    resDos = cuentaCorrienteController.retrotraer_pago(
                                        cuentaCorrienteController,
                                        montoInstructor,
                                        idCuentaCorrienteInstructor,
                                    )
                                    print(
                                        f"se retrotrajo el pago del instructor: {resDos} "
                                    )
                            ...
                            if asociado:
                                if transaccionAsociado:
                                    ...
                                    print(
                                        f"se retrotrajo el pago del asociado: {resUno} "
                                    )
                            db.session.commit()
                            return 5
