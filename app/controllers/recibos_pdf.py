...
class ReciboPDF(FPDF):
    ...
    def itinerarios(self, itinerarios):
        ...
        for itinerario in itinerarios:
            ...
            self.cell(
                0,
                20,
                f"Hora de Salida: {itinerario['horaSalida']} - Código de aeropuerto de salida: {itinerario['codAeroSalida']}",
                1,
                1,
            )
            self.cell(
                0,
                20,
                f"Hora de Llegada: {itinerario['horaLlegada']} - Código de aeropuerto de llegada: {itinerario['codAeroLlegada']}",
                1,
                1,
            )
            self.cell(
                0,
                20,
                f"Tipo de Itinerario: {itinerario['tipoItinerario']} - Cantidad de aterrizajes: {itinerario['cantAterrizajes']}",
                1,
                1,
            )
            self.cell(0, 10, "", 0, 1)  # Espacio entre itinerarios
    ...
    def generar_nombre_archivo(self, id_recibos):
        ...
        return os.path.join(
            os.path.expanduser("~"), "Descargas", f"recibo_{id_recibos}.pdf"
        )  # guarda el pdf en descargas, verificar autorizacion. EL NOMRE ES EL ID DEL RECIBO.
    ...
    def crear_pdf(self, numRecibo):
        ...
        try:
            ...
            user_tiene_recibo = (
                ...
                .filter_by(rol="Asociado", recibos_id=recibo.id_recibos)
                .first()
            )
            id_asociado = user_tiene_recibo.usuarios_id
            asociado = (
                db.session.query(Usuarios).filter_by(id_usuarios=id_asociado).first()
            )
            ...
            msg.attach(
                MIMEText(f"Adjunto encontrarás el recibo número {numRecibo}", "plain")
            )
            ...
            try:
                ...
                print(
                    f"El recibo ha sido enviado por correo electrónico {emailAsociado}"
                )
